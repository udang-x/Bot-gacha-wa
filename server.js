...
// 🤝 ENDPOINT TRADE KARTU AMAN (Diproses di Backend)
app.post('/api/trade', async (req, res) => {
    try {
        const { sender1, sender2, index1, index2 } = req.body;
        
        if (!sender1 || !sender2 || index1 === undefined || index2 === undefined) {
            return res.status(400).json({ success: false, message: 'Data trade tidak lengkap.' });
        }

        const user1 = await getFirebaseUser(sender1);
        const user2 = await getFirebaseUser(sender2);

        if (!Array.isArray(user1.cards)) user1.cards = [];
        if (!Array.isArray(user2.cards)) user2.cards = [];

        if (!user1.cards[index1] || !user2.cards[index2]) {
            return res.status(400).json({ success: false, message: '❌ Trade gagal: Salah satu kartu tidak ditemukan di inventaris.' });
        }

        const card1 = user1.cards[index1];
        const card2 = user2.cards[index2];

        user1.cards[index1] = card2;
        user2.cards[index2] = card1;

        await saveFirebaseUser(sender1, user1);
        await saveFirebaseUser(sender2, user2);

        res.json({
            success: true,
            message: '🎉 Trade berhasil diproses oleh server pusat!'
        });
    } catch (error) {
        console.error("Trade error:", error);
        res.status(500).json({ success: false, message: 'Terjadi kesalahan pada server saat memproses trade.' });
    }
});
